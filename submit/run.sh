...
/execute-step.sh
    ...
    if [ -f "${SPARK_APPLICATION_PYTHON_LOCATION}" ]; then
        ...
        PYSPARK_PYTHON=python3 /spark/bin/spark-submit --master ${SPARK_MASTER_URL} ${SPARK_SUBMIT_ARGS} ${SPARK_APPLICATION_PYTHON_LOCATION} ${SPARK_APPLICATION_PYTHON_LOCATION}
    else
